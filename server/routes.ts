...
const log = (message: string) => console.log(`[DOWNLOAD] ${new Date().toLocaleTimeString()} - ${message}`);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/download", async (req, res) => {
    ...
    try {
      ...
      log(`Video info fetched. Safe title: ${safeTitle}`);
      ...
      if (downloadProcess.stdout) {
        ...
        log('WARNING: downloadProcess.stdout is null. No progress will be logged.');
      }
      ...
      log(`FATAL ERROR in /api/download: ${error}`);
      if (!res.headersSent) {
          res.status(500).json({ error: "A critical error occurred while processing your download request." });
      }
